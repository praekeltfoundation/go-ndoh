        ...
        beforeEach(function() {
            ...
            tester
                ...
                .setup(function(api) {
                    ...
                    api.kv.store['test.chw_test.no_complete_registrations'] = 2;
                    api.kv.store['test.chw_test.no_incomplete_registrations'] = 2;
                })
                .setup(function(api) {
                ...
                it("should increase states_start.no_incomplete metric by 1", function() {
                    return tester
                        ...
                        .check(function(api) {
                            ...
                            assert.deepEqual(metrics['test.chw_test.states_start.no_incomplete'].values, [1]);
                        })
                        .run();
                });
            });
            ...
            describe("when the last state is states_birth_day", function() {
                it("should increase states_birth_day.no_incomplete metric by 1", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.states_birth_day.no_incomplete'].values, [1]);
                        })
                        .run();
                });
            });
            ...
            describe("when the last state is states_birth_day", function() {
                it("and no_incomplete was 1 should increase states_birth_day.no_incomplete metric to 2", function() {
                    return tester
                        .setup(function(api) {
                            api.metrics.stores.test_metric_store = {
                                'test.chw_test.states_birth_day.no_incomplete': { agg: 'last', values: [ 1 ] } 
                            };
                            api.kv.store['test_metric_store.test.chw_test.states_birth_day.no_incomplete'] = 1;
                        })
                        .setup.user.state('states_birth_day')
                        .input.session_event('close')
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.states_birth_day.no_incomplete'].values, [1, 2]);
                        })
                        .run();
                });
            });
            ...
            describe("when the last state is states_end_success", function() {
                it("should not fire a metric", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.states_end_success.no_incomplete'], undefined);
                        })
                        .run();
                });
            });
            ...
            describe("when it is a new user logging on", function() {
                it("should set the last metric value in states_start.no_incomplete to 0", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.states_start.no_incomplete'].values, [1, 0]);
                        })
                        .run();
                });
            });
            ...
            describe("when it is an existing user continuing a session at states_birth_day", function() {
                it("should not fire metric states_birth_day.no_incomplete", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.states_birth_day.no_incomplete'], undefined);
                        })
                        .run();
                });
            });
        ...
        describe("when a new unique user logs on", function() {
            it("should increment the no. of unique users by 1", function() {
                return tester
                    ...
                    .check(function(api) {
                        var metrics = api.metrics.stores.test_metric_store;
                        assert.deepEqual(metrics['test.chw_test.sum.unique_users'].values, [1]);
                        assert.deepEqual(metrics['test.chw_test.percentage_users'].values, [100]);
            ...
            describe("if the user is the pregnant woman", function() {
                it("should set id type, ask for their id number", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.percent_incomplete_registrations'].values, [60]);
                            assert.deepEqual(metrics['test.chw_test.percent_complete_registrations'].values, [40]);
                        })
                        .run();
                });
            });
            ...
            describe("if the phone used is not the mom's", function() {
                it("should save msg language, thank them and exit", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.avg.sessions_to_register'].values, [5]);
                            assert.equal(metrics['test.chw_test.states:end_success.no_incomplete'], undefined);
                            assert.deepEqual(metrics['test.chw_test.sum.json_to_jembi_success'].values, [1]);
                        })
                ...
                it("should save msg language, thank them and exit", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.avg.sessions_to_register'].values, [5]);
                            assert.deepEqual(metrics['test.chw_test.percent_incomplete_registrations'].values, [25]);
                            assert.deepEqual(metrics['test.chw_test.percent_complete_registrations'].values, [75]);
                            assert.deepEqual(metrics['test.chw_test.sum.json_to_jembi_success'].values, [1]);
                        })
                        .check.reply.ends_session()
                        .run();
            ...
            describe("if the jembi send fails", function() {
                it.skip("should fire a fail metric", function() {
                    return tester
                        ...
                        .check(function(api) {
                            var metrics = api.metrics.stores.test_metric_store;
                            assert.deepEqual(metrics['test.chw_test.sum.json_to_jembi_fail'].values, [1]);
                        })
                        .check.reply.ends_session()
                        .run();
                });
